...
# Permission checkers
can_read_raw_logs = make_permission_checker("raw_logs.read")
can_delete_raw_logs = make_permission_checker("raw_logs.delete")
...
async def create_raw_log(
    ...
    _: bool = Depends(can_read_raw_logs),
):
...
async def delete_raw_log(
    ...
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_authenticated_user),
    _: bool = Depends(can_delete_raw_logs),
):
    result = await db.execute(select(RawLog).where(RawLog.id == log_id))
    log = result.scalars().first()
    if not log:
