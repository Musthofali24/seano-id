...
# Permission checkers
can_create_sensor_types = make_permission_checker("sensor_types.create")
can_read_sensor_types = make_permission_checker("sensor_types.read")
can_update_sensor_types = make_permission_checker("sensor_types.update")
can_delete_sensor_types = make_permission_checker("sensor_types.delete")
...
async def create_sensor_type(
    ...
    _: bool = Depends(can_create_sensor_types),
):
...
async def get_sensor_types(
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_authenticated_user),
    _: bool = Depends(can_read_sensor_types),
):
...
async def update_sensor_type(
    ...
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_authenticated_user),
    _: bool = Depends(can_update_sensor_types),
):
    result = await db.execute(select(SensorType).where(SensorType.id == sensor_type_id))
    sensor_type = result.scalars().first()
    if not sensor_type:
...
async def delete_sensor_type(
    ...
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_authenticated_user),
    _: bool = Depends(can_delete_sensor_types),
):
    result = await db.execute(select(SensorType).where(SensorType.id == sensor_type_id))
    sensor_type = result.scalars().first()
    if not sensor_type:
