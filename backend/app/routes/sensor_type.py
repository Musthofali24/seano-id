...
async def create_sensor_type(sensor_type: SensorTypeCreate, db: AsyncSession = Depends(get_db)):
...
async def get_sensor_types(db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(SensorType))
    return result.scalars().all()

# Get Data by Id
@router.get("/{sensor_type_id}", response_model=SensorTypeResponse)
async def get_sensor_type(sensor_type_id: int, db: AsyncSession = Depends(get_db)):
...
async def update_sensor_type(sensor_type_id: int, update: SensorTypeUpdate, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(SensorType).where(SensorType.id == sensor_type_id))
    sensor_type = result.scalars().first()
    if not sensor_type:
        raise HTTPException(status_code=404, detail="Sensor type not found")
...
async def delete_sensor_type(sensor_type_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(SensorType).where(SensorType.id == sensor_type_id))
    sensor_type = result.scalars().first()
    if not sensor_type:
        raise HTTPException(status_code=404, detail="Sensor type not found")
