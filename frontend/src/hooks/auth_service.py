...
async def create_user(
    email: str, password: str, full_name: str = None, db: AsyncSession = None
) -> User:
    """Create new user"""
    ...
    # Create user
    user = User(email=email, full_name=full_name, password_hash=hashed_password)
...
async def set_user_credentials(
    token: str, username: str, password: str, db: AsyncSession
):
...
async def create_user_with_verification(
    email: str, password: str, full_name: str, db: AsyncSession
) -> User:
...
async def get_user_role(user: User, db: AsyncSession) -> str:
    """Get primary role for a user by querying user_roles and roles directly"""
    if not user.id:
        return None

    # Query roles by joining user_roles -> role
